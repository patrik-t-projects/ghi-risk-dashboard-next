  ...
  useEffect(() => {
    ...
    async function draw() {
      try {
        ...
        // The export combines observation and forecast sampling times. Observation-only
        // rows must not break the hourly forecast lines (and vice versa).
        const forecastRows = station.rows.filter(row => row.values.some(value => value !== null));
        const measuredRows = station.rows.filter(row => row.measured !== null);
        ...
        traces.push({ type: "scatter", mode: "lines", name: "Actual GHI", legendgroup: "actual",
          x: measuredRows.map(row => row.time.replace(/Z$/, "")), y: measuredRows.map(row => row.measured),
        ...
        await Plotly.react(element, traces, {
          ...
          hovermode: "x unified", uirevision: `${station.id}-${model}`, showlegend: true,
    ...
    void draw();
    return () => { disposed = true; observer?.disconnect(); cancelAnimationFrame(frame); purge?.(); };
  }, [data, station, model, hiddenRuns, members, attempt]);
