...
export default function SwitzerlandBeta({ mode, onModeChange }: { mode: "today" | "history"; onModeChange: (mode: "today" | "history") => void }) {
  ...
  return <div className="h-[calc(100dvh-4rem)] overflow-y-auto p-4 sm:p-6">
    ...
    <section className={styles.mapCard} aria-label="Switzerland station map">
      <div className="absolute left-5 top-5 z-10 flex items-center gap-4 text-xs text-slate-300"><span className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-red-500" />GHI weather stations</span><span className="flex items-center gap-2"><span className="h-2.5 w-4 rounded-sm bg-sky-500/70" />Lakes</span></div>
