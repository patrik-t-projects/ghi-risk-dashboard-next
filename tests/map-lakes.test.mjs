...
test('map uses the complete swissTLMRegio lake layer', () => {
  assert.match(lakes.source, /swisstopo.*swissTLMRegio 2025/i);
  assert.equal(feature.geometry.type, 'MultiPolygon');
  assert.equal(feature.properties.featureCount, 3420);
  assert.equal(polygons.length, 3420);
});

test('official lake geometry includes Zugersee and cross-border lakes', () => {
